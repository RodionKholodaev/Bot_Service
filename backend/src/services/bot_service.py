...
class BotService:
    ...
    async def create_bot(self, current_user: User, body: BotCreate):
        ...
        bot = Bot(
            ...
            # Дистанции и порога у нас нет (отсюда и пустая trailing_config): захардкоженные
            # числа freqtrade делит на плечо, при x10 трейлинг взводился бы почти сразу и
            # обрезал каждый выигрыш вместо пользовательского take-profit, оставляя убытки
            # полноразмерными. Включать имеет смысл только вместе с осмысленными числами и
            # после разбора аварийных выходов: с "stoploss_on_exchange" трейлинг заставляет
