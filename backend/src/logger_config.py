...
def extra_fields(record: logging.LogRecord) -> dict:
    ...
    return {key: value for key, value in record.__dict__.items() if key not in STANDARD_LOG_RECORD_ATTRS}
...
def setup_logging():
    ...
    # какой уровень показывать, какие хендлеры использовать
